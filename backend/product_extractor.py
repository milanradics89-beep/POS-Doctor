...
def _availability(value: Any) -> str:
    ...
    if "outofstock" in text or "out_of_stock" in text:
        return "out_of_stock"
    if "limited" in text or "limitedavailability" in text:
        return "limited"
...
def _extract(parser: _HTMLParser, url: str) -> dict[str, Any] | None:
    ...
    name = product.get("name") or parser.meta.get("og:title")
    if not name:
        return None
    ...
    try:
        price_number = float(re.sub(r"[^0-9.,-]", "", str(price)).replace(",", ".")) if price is not None else None
    except ValueError:
        price_number = None
    ...
    return {
        ...
        "retailer": urlparse(canonical).hostname,
        "evidence": evidence,
        "qualityScore": min(1.0, 0.45 + 0.15 * len(evidence)),
    }
...
async def resolve_product(request: ProductResolveRequest):
    ...
    try:
        ...
        raise HTTPException(502, "Product page returned an HTTP error.") from exc
    except httpx.HTTPError as exc:
        raise HTTPException(504, "Product page could not be fetched.") from exc
    ...
    if not product:
        raise HTTPException(422, "No valid Product structured data was found.")
    return product
