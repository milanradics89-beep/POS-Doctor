...
def test_health():
 r=client.get('/health'); assert r.status_code==200; assert r.json()['responseFormat']=='scene_analysis_v1'

def test_rejects_wrong_response_format():
 r=client.post('/v1/analyze',json={'imageUri':'data:image/jpeg;base64,'+'a'*32,'responseFormat':'old'}); assert r.status_code==400

def test_requires_api_key():
 with patch.dict('os.environ',{},clear=True): r=client.post('/v1/analyze',json={'imageUri':'data:image/jpeg;base64,'+'a'*32})
 assert r.status_code==503

def test_analyze_passes_image_and_policy():
 mock=Mock(); mock.responses.create.return_value=Mock(output_text=json.dumps(valid_output()))
