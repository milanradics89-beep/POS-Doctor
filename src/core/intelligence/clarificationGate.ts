...
export function buildClarificationGate(scene: SceneModel, task: PlannedTask, analysis: DomainAnalysis): ClarificationResult {
  ...
  const preferredStyles = task.constraints.preferredStyles;
  if (scene.domain === 'room' && task.shoppingRequired && (!Array.isArray(preferredStyles) || preferredStyles.length === 0)) questions.push({ id: 'style', text: 'Milyen stílust szeretnél?', type: 'single_select', options: ['Modern','Minimalista','Skandináv','Klasszikus','Industrial','Rád bízom'], required: false });
