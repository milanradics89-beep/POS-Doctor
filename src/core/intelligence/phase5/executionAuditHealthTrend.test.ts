...
describe('compareExecutionAuditHealth', () => {
  it('calculates rate deltas between periods', () => {
    const current = [
      { executionId: '1', idempotencyKey: '1', actionType: 'shop', status: 'SUCCEEDED' as const, recordedAt: 'a' },
      { executionId: '2', idempotencyKey: '2', actionType: 'shop', status: 'SUCCEEDED' as const, recordedAt: 'b' },
      { executionId: '3', idempotencyKey: '3', actionType: 'shop', status: 'FAILED' as const, recordedAt: 'c' },
    ];
    const previous = [
      { executionId: '4', idempotencyKey: '4', actionType: 'shop', status: 'SUCCEEDED' as const, recordedAt: 'd' },
      { executionId: '5', idempotencyKey: '5', actionType: 'shop', status: 'FAILED' as const, recordedAt: 'e' },
    ];
